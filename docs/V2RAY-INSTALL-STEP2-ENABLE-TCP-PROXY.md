...
本指南用于在云服务器上配置 V2Ray TCP 透明代理,使 WireGuard 客户端的 TCP 流量(HTTP/HTTPS)自动通过代理。
...
脚本会自动：
- 备份现有配置
- 使用 jq 解析当前配置中的上游服务器信息
- 生成新的配置文件（添加 TCP 透明代理支持）
- 验证配置语法
- 重启 V2Ray 服务
- 配置 iptables 规则
- 保存防火墙规则
...
- **智能配置解析**：自动从现有配置读取上游服务器信息
- **配置验证**：验证 V2Ray 配置语法，错误时自动回滚
- **完整规则配置**：自动配置 iptables NAT 规则
- **状态检查**：验证端口监听和规则生效

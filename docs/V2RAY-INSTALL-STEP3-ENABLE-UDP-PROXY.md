...
本指南用于在云服务器上配置 V2Ray UDP 透明代理,使 WireGuard 客户端的 UDP 流量(如 QUIC、DNS)也能通过代理。
...
- 生成新的配置文件（添加 UDP 透明代理支持）
- 验证配置语法
- 重启 V2Ray 服务
- 加载 TPROXY 内核模块
- 配置 iptables mangle 规则
- 配置策略路由
- 创建持久化脚本

### 脚本功能特性

- **TPROXY 支持**：自动加载和配置 TPROXY 模块
- **策略路由**：配置 fwmark 路由表
- **完整验证**：检查端口监听、规则生效、路由配置
- **持久化**：创建网络接口启动脚本
